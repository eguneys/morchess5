...
export const format_Z_a1 = (text: string) => {
    let m = text.match(/X = (\w*), Y = (\w*), Z = (\w*)/)!
    return [m[1], m[2], m[3]].join(' ')
}

export const format_X_a1 = (text: string) => {
    let m = text.match(/X = (\w*)/)!
    return m[1]
}
export const format_Ls_a1 = (text: string) => {
    let m = text.match(/Ls = \[([^\]]*)\]/)!
    return parse_comma_with_paren(m[1])
}


function parse_comma_with_paren(str: string) {
    let res = []
    let is_open = false
...
export async function _prolog_query(program: string, query: Record<string, string>) {
    try {
